...
function find_opponent(time){
  $.ajax({
    type: "POST",
    url: "/game/find_opponent/",
    data: {
    ...
    success: function(data){
      if (data){
        arr = data.split(";");
        document.querySelector('.play-player__name__last').innerHTML = arr[0];
...
$("#play__start").click(function(){
  ...
  var time = randomInteger(1500, 6000)
  find_opponent(time)

  setTimeout(function(){
    document.querySelector('.play-player__img__last').style.display = 'initial'
